...
class CookieRefresher:
    ...
    async def refresh(self):
        ...
        try:
            async with async_playwright() as p:
                ...
                # ── s_v_web_id presence check ─────────────────────────
                # s_v_web_id is the device fingerprint cookie sourced from
                # live.douyin.com subdomain.  It is required for every
                # Douyin API call (as the "fp" parameter).  Without it,
                # many API endpoints return CAPTCHA or empty responses.
                has_s_v_web_id = "s_v_web_id" in new_cookies
                if not has_s_v_web_id:
                    print(
                        "[CookieRefresher] WARNING: s_v_web_id cookie missing "
                        "from extracted cookies — "
                        "live.douyin.com may have changed its fingerprinting",
                        flush=True,
                    )
                    TelegramNotifier().send(
                        "douyin s_v_web_id cookie MISSING — "
                        "live.douyin.com may have changed its fingerprinting",
                        state="s_v_web_id_missing",
                    )
                ...
                test_ok = await self._test_cookie(
                    auth.cookie_str if saved else cookie_str
                )
    ...
    async def _test_cookie(self, cookie_str: str) -> bool:
        """Test a cookie string against Douyin's homepage.
        ...
        try:
            ...
            print(
                f"[CookieRefresher] Test ERROR: {e}",
                flush=True,
            )
            return False
...
async def main():
    ...
    if success:
        ...
        notifier.send(
            f"douyin cookie refreshed ({tag}) "
            f"[{datetime.now().strftime('%H:%M')}]",
            state=None,
        )
    else:
        notifier.send(
            "douyin cookie refresh FAILED — "
            "check server logs",
            state="dead",
        )
    ...
    while True:
        ...
        if success:
            ...
            notifier.send(
                "douyin cookie refresh FAILED — session may be dead, "
